        ...
        <section className="min-h-screen">
            <div className="bg-gradient-to-r from-purple-500 to-blue-500 py-24 px-4">
                <form className="max-w-3xl mx-auto relative" onSubmit={onSubmit}>
                    <input className="w-full rounded-md text-xl p-4 outline-none" 
                        type="text" 
                        value={inputValue} 
                        onChange={(e) => setInputValue(e.target.value)} />
                    <button className="p-4 text-gray-500 hover:text-black absolute right-0 top-1/2 -translate-y-1/2" type='submit'>
            ...
            <div>
                <div className='p-8 max-w-5x1 mx-auto'>
                    {isLoading ? <div className="flex justify-center"><LoadingSpinner /></div> : <CardList data={postData} />}
                    {!isLoading && postData.length === 0 ? <div className="text-center">No result found</div> : null}
                </div>
            </div>
        </section>
    )
