import { PAGINATION_PAGE, POSTS_PER_PAGE } from 'const/const';
...
const Pagination = ({current, total}: PageinationProps) => {
    ...
    return (
        <div className='flex gap-1'>
            ...
            {
                Array.from(Array(PAGINATION_PAGE), (_, index) => current - index - 1).reverse().map(pageIndex => (
                    pageIndex > 0 ? (
                        <PaginationButton key={pageIndex} to={pageIndex}>{pageIndex}</PaginationButton>
                    ) : null
                ))
            }
            ...
            {
                Array.from(Array(PAGINATION_PAGE), (_, index) => current + index + 1).map(pageIndex => (
